...
fn main() {
    ...
    let rdd1 = sc.new_from_list(vec![vec![1], vec![2], vec![3], vec![4]]);
    let rdd2 = sc.map(rdd1, |x| 2 * x);
    let rdd3 = sc.filter(rdd2, |&x| x > 5);
